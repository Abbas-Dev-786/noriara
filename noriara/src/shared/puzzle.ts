...
export type PuzzleMechanic = 'core';
...
type ArchetypeBuilder = (rng: RandomFn, band: DifficultyBand, _index: number) => {
  targets: PuzzleShape[];
  hazards: PuzzleShape[];
  meta: Omit<PuzzleLayoutMeta, 'targetCount' | 'hazardCount'>;
};

function getBand(index: number): DifficultyBand {
...
function pickRandom<T>(rng: RandomFn, items: { item: T; weight: number }[]): T {
...
const buildBounceWeave: ArchetypeBuilder = (rng, band, _index) => {
...
const buildHazardOrbit: ArchetypeBuilder = (rng, band, _index) => {
...
const buildDelayedCatch: ArchetypeBuilder = (rng, band, _index) => {
...
const buildWideSweep: ArchetypeBuilder = (rng, band, _index) => {
  const targetCount =
...
const buildTightCluster: ArchetypeBuilder = (rng, band, _index) => {
  const targetCount = band === 'medium' ? 3 : randomInt(rng, 3, 4);
...
function selectArchetype(rng: RandomFn, band: DifficultyBand): ArchetypeBuilder {
  if (band === 'tutorial') {
...
function validateLayout(targets: PuzzleShape[], hazards: PuzzleShape[], band: DifficultyBand, archetype: PuzzleArchetype): string | null {
