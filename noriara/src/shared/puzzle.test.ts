import { generatePuzzlesForSeed, PROTOTYPE_MECHANICS_REGISTRY } from './puzzle';
...
function notDeepEqual(a: unknown, b: unknown, msg?: string) {
  if (JSON.stringify(a) === JSON.stringify(b)) {
  ...
  for (const puzzle of puzzles) {
    ...
    for (let i = 0; i < puzzle.targets.length; i++) {
      for (let j = i + 1; j < puzzle.targets.length; j++) {
        const d = Math.hypot(
          puzzle.targets[i]!.x - puzzle.targets[j]!.x,
          puzzle.targets[i]!.y - puzzle.targets[j]!.y
        );
    ...
    for (let i = 0; i < puzzle.hazards.length; i++) {
      for (let j = i + 1; j < puzzle.hazards.length; j++) {
        const d = Math.hypot(
          puzzle.hazards[i]!.x - puzzle.hazards[j]!.x,
          puzzle.hazards[i]!.y - puzzle.hazards[j]!.y
        );
...
function testAllLayoutsUseEnabledCoreMechanics() {
  ...
  assert(
    Object.values(PROTOTYPE_MECHANICS_REGISTRY).every((enabled) => enabled === false),
    'prototype mechanics should remain disabled by default'
  );
}
  ...
  for (const puzzle of puzzles) {
    ...
    if (archetype === 'tight-cluster') {
      ...
      const maxDistance = Math.max(
        ...puzzle.targets.map((target) => Math.hypot(target.x - clusterCenter.x, target.y - clusterCenter.y))
      );
