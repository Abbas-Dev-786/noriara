...
export function validateOfficialRunPayload(payload) {
    ...
    for (const attempt of attempts) {
        ...
        const event = consumeAttemptEvent(attempt, solveEvents, failureEvents, solveIndex, failureIndex);
        if (!event) {
...
function isAttemptTelemetryValid(attempt) {
    ...
    const smoothed = smoothPath(attempt.points.map((point) => ({ x: point.x, y: point.y })), 2);
...
function simulateAttempt(attempt, targets, hazards) {
    const baseGesture = normalizePath(smoothPath(attempt.points.map((point) => ({ x: point.x, y: point.y })), 2), STEP_SIZE);
    ...
    for (let step = 1; step <= MAX_SIMULATION_STEPS; step++) {
        ...
        const from = baseGesture[baseIndex];
        const to = baseGesture[baseIndex + 1];
